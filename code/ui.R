...
shinyUI(fluidPage(
  titlePanel("Taipei House Rental 2018"),
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      numericInput("rooms", "房:", value=0, min=0),
      numericInput("halls", "廳:", value=0, min=0),
      numericInput("baths", "衛:", value=0, min=0),
      radioButtons("car", "車位", choices=c("有"=1, "無"=0), selected=0),
      sliderInput("price", "價格", min=10000, max=500000, value=15000, step=5000)
    ),
    mainPanel(
      tabsetPanel(
        tabPanel("Searching", DT::dataTableOutput("out_d")),
